...
module.exports = async ({ getNamedAccounts, deployments, network }) => {
  ...
  console.log(
    `Setting owner of root node to root contract (tx: ${tx.hash})...`
  );
  await tx.wait();
  ...
  switch (rootOwner) {
    case deployer:
      ...
      console.log(
        `Transferring root ownership to final owner (tx: ${tx.hash})...`
      );
      await tx.wait();
    case owner:
      if (!(await root.controllers(owner))) {
        ...
        console.log(
          `Setting final owner as controller on root contract (tx: ${tx.hash})...`
        );
        await tx.wait();
      }
      break;
